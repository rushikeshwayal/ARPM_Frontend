...
export default function Inbox() {
  ...
  return (
    <div>
      ...
      {messages.length === 0 && (
        <div className="bg-white p-6 rounded-xl shadow-md text-gray-500">No messages</div>
      )}
      ...
      {messages.map(msg => (
        ...
        >
          ...
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 bg-purple-600">
            {getEmail(msg.sender_id).charAt(0).toUpperCase()}
          </div>

          <div className="flex-1 min-w-0">
            <p className="font-semibold text-gray-800 truncate">
              <span className="text-sm font-bold text-purple-600">From : </span>
          ...
          >
            ...
            <div className="flex items-center gap-3 px-6 py-4 border-b">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold bg-purple-600">
                {getEmail(selected.sender_id).charAt(0).toUpperCase()}
              </div>
              <div>
                <p className="font-medium text-gray-800">
                  <span className="text-xs font-bold text-purple-600">From: </span>
                  {getEmail(selected.sender_id)}
                </p>
